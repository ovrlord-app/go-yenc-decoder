...
func TestDecodeStopsAtYEndWithTrailingData(t *testing.T) {
	...
	d, err := Decode(strings.NewReader(input), DecodeWithPrefixData())
	if err != nil {
		t.Fatalf("decode init failed: %v", err)
	}
	...
	n, err := d.Read(buf)
	if n != 0 || err != io.EOF {
		t.Fatalf("expected EOF after yend; got n=%d err=%v", n, err)
	}
}
