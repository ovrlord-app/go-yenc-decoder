...
// Max number of bytes per line (ends in LF and includes the LF) when decoding yEnc data stream.
// Set to 256 KiB so the ring buffer can hold many complete yEnc lines, avoiding boundary issues
// that caused stalling with the original 4 KiB default.
